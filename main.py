...
def cell_options(grid,i,j, verbose = 0):
    ...
    if grid[i,j]!=0: # there is a digits at the location
        ...
        for k in range(9):
            ...
            cell_options = np.multiply(np.multiply(col_options,row_options),box_options)
    return cell_options
...
def direct_solver(grid, verbose = 0):
    ...
    returns:
        is_solvable : boolean = True if grid is solved, False if grid is unsolvable
        has_conclusion : boolean False if function was able to determine if grid is solvable   
    ...
    while sum(sum(sum(potential_hyp)))!=0:
        ...
        for i,j in product(range(9),range(9)): #We going to try every posible hypothesis until we found a directly solvable solution
            if sum(options[i,j,:])!=0 and not np.all(potential_hyp[i,j,:]==0):
                ...
                grid,nb_empty,options = input_cell(grid,i,j,m+1,nb_empty)
                if verbose >=2 : input(f"{i},{j} :       trying a {m+1}")
                break
